...
const CourseDetailPage = () => {
  ...
  // Check for completion and award badge (Moved here to avoid "Rendered more hooks" error)
  const priceObj = course?.price;
  const hasPriceObj = !!(priceObj && (priceObj.sale != null || priceObj.original != null));
  const salePrice = hasPriceObj && priceObj.sale != null ? Number(priceObj.sale) : null;
  const originalPrice = hasPriceObj && priceObj.original != null ? Number(priceObj.original) : null;
    ...
    const checkBadge = async () => {
      try {
        ...
        if (data.success) {
          ...
          if (badgeExists) setHasBadge(true);
        }
      } catch (e) {
  ...
  useEffect(() => {
    ...
    if (totalChaptersCount > 0 && completedCount === totalChaptersCount && !hasBadge) {
      const awardBadge = async () => {
        try {
          ...
          if (data.success) {
            setHasBadge(true);
            if (data.newlyAwarded) setShowBadgeModal(true);
          }
        } catch (error) {
                ...
                <div>
                  ...
                  <div className={courseDetailStyles.progressBar}>
                    <div
                      ...
                      style={{
                        width: `${hasBadge ? 100 : (completedChapters.size /
                          (course.lectures?.flatMap((l) => l.chapters || [])
                            .length || 1)) *
                          100
                          }%`,
                  ...
                  <div className="mt-2 text-sm">
                    {hasBadge ? (
                      ...
                      <span className="text-gray-400">{completedChapters.size} lessons completed</span>
                    )}
                  </div>

                  {/* View Badge Button */}
                  {(Math.round((completedChapters.size / (course.lectures?.flatMap((l) => l.chapters || []).length || 1)) * 100) === 100 || hasBadge) && (
                    <button
                      onClick={() => setShowBadgeModal(true)}
                      className="flex items-center gap-2 mt-3 w-full justify-center px-4 py-2 bg-gradient-to-r from-amber-500 to-yellow-600 hover:from-amber-400 hover:to-yellow-500 text-black font-bold text-sm rounded-lg transition-all shadow-lg hover:shadow-amber-500/30"
                    >
                      <Award size={16} />
                      View Badge
                    </button>
