...
import { Search, User, Star, StarHalf, X, SmilePlus, Award } from "lucide-react";
...
const CoursePage = () => {
    ...
    useEffect(() => {
        ...
        // Need user ID. useUser hook provides it.
    }, [isSignedIn]);

    // Separate effect for badges since we need user object from hook
    const { user } = useUser();
    useEffect(() => {
                ...
                const mapped = raw.map((c) => ({
                    ...
                    avgRating:
                        (typeof c.avgRating === "number" && c.avgRating > 0)
                            ? c.avgRating
                            : (typeof c.rating === "number" ? c.rating : (parseFloat(c.rating) || 0)),
                    totalRatings:
        ...
        <div className={coursePageStyles.pageContainer}>
            ...
            <div className={coursePageStyles.headerContainer}>
                <div className={coursePageStyles.headerTransform}>
                    <h1 className={coursePageStyles.headerTitle}>LEARN AND GROW</h1>
                </div>
                ...
                <div className={coursePageStyles.searchContainer}>
                    ...
                    <div className={coursePageStyles.searchInputContainer}>
                        <div className={coursePageStyles.searchIconContainer}><SearchIcon /></div>
                        <input
                            type="text"
                                                ...
                                                <div className={coursePageStyles.courseImageContainer}>
                                                    {course.icon ? (
                                                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-violet-500/10 to-cyan-500/10 group-hover:from-violet-500/20 group-hover:to-cyan-500/20 transition-all duration-300">
                                                            <course.icon className="w-16 h-16 sm:w-20 sm:h-20 text-violet-400 group-hover:text-cyan-400 transition-colors duration-300 drop-shadow-[0_0_15px_rgba(139,92,246,0.3)]" />
                                                        </div>
                                                    ) : (
                                                        course.image ? (
                                                            <img
