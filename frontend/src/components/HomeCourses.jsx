...
import { ArrowRight, Star, User, Award } from 'lucide-react';
...
const HomeCourses = () => {
    ...
    // Fetch badges to identify completed courses
    const { user } = useUser();
    useEffect(() => {
        ...
        const fetchBadges = async () => {
            try {
                ...
                if (data.success) {
                    ...
                    const badgeSet = new Set(data.badges.map(b => b.courseId));
                    setEarnedBadges(badgeSet);
                }
    ...
    useEffect(() => {
        ...
        fetch(`${API_BASE}/api/course/public?home=true`).then(
            async (res) => {
                if (!res.ok) {
            ...
            <div className={homeCoursesStyles.mainContainer}>
                ...
                {loading ? (
                    ...
                    <div className={homeCoursesStyles.coursesGrid}>
                        {courses.map((course, index) => {
                            ...
                            return (
                                <div key={`${course.id}-${index}`} onClick={() => handleCourseClick(course.id)} className={homeCoursesStyles.courseCard}>
                                    {earnedBadges.has(course.id) && (
                                    ...
                                    <div className={homeCoursesStyles.imageContainer}>
                                        {course.icon ? (
                                            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-violet-500/10 to-cyan-500/10 group-hover:from-violet-500/20 group-hover:to-cyan-500/20 transition-all duration-300">
                                                <course.icon className="w-20 h-20 text-violet-400 group-hover:text-cyan-400 transition-colors duration-300 drop-shadow-[0_0_15px_rgba(139,92,246,0.3)]" />
                                            </div>
                                        ) : (
                ...
                <div className={homeCoursesStyles.ctaContainer}>
                    <div className={homeCoursesStyles.ctaWrapper}>
                        <span className={homeCoursesStyles.ctaGlow} style={{ zIndex: 0, background: "conic-gradient(from 0deg, rgba(236,72,153,0.9), rgba(99,102,241,0.9), rgba(139,92,246,0.9), rgba(236,72,153,0.9))", filter: "blur(5px)", opacity: 0.8, }} />
                        <button onClick={handleBrowseClick} className={homeCoursesStyles.ctaButton} style={{ background: "linear-gradient(180deg, rgba(255,255,255,0.06), rgba(255,255,255,0.02))", }}>
